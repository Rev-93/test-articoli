...
const DB_NAME = 'TestArticoliDB';
const DB_VERSION = 1;
const IMAGES_FOLDER = 'images';

const JSONBIN_CONFIG = {
    apiKey: '$2a$10$0j1eRl8lySp7UVE2K9cLduma/RWSvxOzlzrkUje604JfzCbVaWKeu',
    binId: '69ba50d0aa77b81da9f55695'
};
    ...
    return new Promise(async (resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains('tests')) {
                ...
                store.createIndex('operator_name', 'operator_name', { unique: false });
...
async function importFromJsonBin() {
    const remoteTests = await loadFromJsonBin();
    if (remoteTests.length === 0) return;
...
async function syncToJsonBin(tests) {
    if (!JSONBIN_CONFIG.apiKey || !JSONBIN_CONFIG.binId) return;
